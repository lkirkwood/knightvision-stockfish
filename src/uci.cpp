    ...
    // These commands must not be used during a search!
      ...
      sync_cout
          << "\nStockfish is a powerful chess engine for playing and analyzing."
             ...
             "License."
             "\nStockfish is normally used with a graphical user interface "
             "(GUI) and implements"
...
std::string UCIEngine::run_cmd(std::string cmd) {
  ...
  auto _consume_info_string = [&](std::string_view str) {
    for (auto &line : split(str, "\n")) {
      if (!is_whitespace(line)) {
        response << "info string " << line << '\n';
      }
    }
  };
  ...
  // from pondering to the normal search.
    ...
    response << "id name " << engine_info(true) << "\n"
             << engine.get_options() << sync_endl;

    response << "uciok" << sync_endl;
  }
    ...
    // send info strings after the go command is sent for old GUIs and
    // python-chess
    _consume_info_string(engine.numa_config_information_as_string());
    _consume_info_string(engine.thread_allocation_information_as_string());
    go(is);
  } else if (token == "position")
    position(is);
  else if (token == "ucinewgame")
  ...
  // These commands must not be used during a search!
    ...
    response << engine.visualize() << sync_endl;
  else if (token == "eval")
    engine.trace_eval();
  else if (token == "compiler")
    response << compiler_info() << sync_endl;
  else if (token == "export_net") {
    std::pair<std::optional<std::string>, std::string> files[2];
